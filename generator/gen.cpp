...
void logAndExit(const string &s) {
...
bool makeScaleFree(vector<vector<bool> > &adjacent, int &numEdges, int m, const Rng &rng) {
    ...
    for (i = m0; i < n; i++) {
        ...
        for (j = 0; j < m; j++) {
            ...
            v = selectLinkRoulette(tempDeg, i, totalTempDeg, rng);
...
bool getPairs(vector<int> &pointPairs, int n, int d, const Rng &rng) {
    ...
    for (i = 0; i < (int) points.size(); i++) points[i] = i;
...
bool graphIsDRegular(vector<int> &pointPairs, int n) {
    int i, u, v;
    vector<int> deg(n, 0);
    for (i = 0; i < (int) pointPairs.size() / 2; i++) {
        u = pointPairs[i * 2] % n;
        v = pointPairs[i * 2 + 1] % n;
        deg[u]++;
        deg[v]++;
    }
    for (i = 1; i < (int) deg.size(); i++) {
...
void makeDRegularGraph(vector<vector<bool> > &adjacent, int &numEdges, int d, const Rng &rng) {
    ...
    while (true) {
        ...
        if (getPairs(pointPairs, n, d, rng)) {
            //Check output is d-regular. If so we can end
            if (graphIsDRegular(pointPairs, n)) {
                break;
            }
        }
    }
    ...
    while (failedCount < maxFails) {
        ...
        for (col = 0; col < k; col++) {
            for (i = 0; i < n; i++) {
                v = perm[i];
                if (colour[v] == -1) {
                    if ((hasNeighbourWithDiffColourToCol(v, col, colour, adjList))) {
        ...
        if (numAssigned < m && !failedColourAssignment) {
            ...
            for (i = 0; i < n; i++) {
                ...
                if (colour[v] == -1) {
                    ...
                    col = getSuitableColour(v, colour, adjList, k, rng);
                    if (col >= 0) {
                        colour[v] = col;
                        numAssigned++;
                        if (numAssigned == m) {
        ...
        if (!failedColourAssignment) {
            ...
            comment = "Creation-Successful-After-(" + to_string(failedCount + 1) + "/" + to_string(maxFails) +
                      ")-Attempts";
            return true;
        }
...
int main(int argc, char *argv[]) {
    ...
    try {
        for (i = 1; i < argc; i++) {
            if (strcmp("-n", argv[i]) == 0) {
                n = (int) strtol(argv[++i], nullptr, 10);
            } else if (strcmp("-p", argv[i]) == 0) {
                propPrecol = strtod(argv[++i], nullptr);
            } else if (strcmp("-k", argv[i]) == 0) {
                k = (int) strtol(argv[++i], nullptr, 10);
            } else if (strcmp("-R", argv[i]) == 0) {
                graphType = 1;
                edgeProb = strtod(argv[++i], nullptr);
            } else if (strcmp("-S", argv[i]) == 0) {
                graphType = 2;
                linksPerNewNode = (int) strtol(argv[++i], nullptr, 10);
            } else if (strcmp("-D", argv[i]) == 0) {
                graphType = 3;
                fixedDegree = (int) strtol(argv[++i], nullptr, 10);
            } else if (strcmp("-A", argv[i]) == 0) {
                adjPrecolAllowed = false;
            } else if (strcmp("-s", argv[i]) == 0) {
                seed = (int) strtol(argv[++i], nullptr, 10);
            } else if (strcmp("-d", argv[i]) == 0) {
                precolourOption = (int) strtol(argv[++i], nullptr, 10);
    ...
    if (numPrecol < k) {
        logAndExit("Error: This combination of input parameters is illegal: -k = " + to_string(k) +
                   " is larger than the reuired number of precoloured vertices = " + to_string(int(propPrecol * n)) +
                   ". Ending...\n");
    }
    if (edgeProb < 0.0 || edgeProb > 1.0 || numPrecol < 0 || numPrecol > n || k > n || linksPerNewNode < 0 ||
        linksPerNewNode >= n || fixedDegree < 0 || fixedDegree >= n) {
        logAndExit("Error: Invalid numerical values in the input statement. Ending...\n");
    }
    if (graphType == 3 && (fixedDegree * n) % 2 == 1) {
        logAndExit("Error: -D = " + to_string(fixedDegree) + " and -n = " + to_string(n) +
                   ". Can only produce d-regular graphs when the product of these two values is even. Ending...\n");
    }
    ...
    if (graphType == 1) {
        makeRandomGraph(adjacent, numEdges, edgeProb, rng);
    } else if (graphType == 2) {
        makeScaleFree(adjacent, numEdges, linksPerNewNode, rng);
    } else {
        makeDRegularGraph(adjacent, numEdges, fixedDegree, rng);
    }
    ...
    if (precolourOption == 2) {
        ...
        shuffle(perm.begin(), perm.begin() + k, rng);
    ...
    str += to_string(numEdges / ((n * (n - 1)) / 2.0)) + "\t" + to_string(seed) + "\t" + to_string(precolourOption) +
           "\t" + to_string(adjPrecolAllowed) + "\t" + comment + "\n";
    ofstream resultsLog("genLog.txt", ios::app);
    cout << str;
    resultsLog << str;
    resultsLog.close();
